...
build-debug build-release: build-%: cmake-%
	@cmake --build build_$* -j $(NPROCS) --target chess_game

# Test
.PHONY: test-debug test-release
test-debug test-release: test-%: build-%
	@cmake --build build_$* -j $(NPROCS) --target chess_game_unittest
	@cmake --build build_$* -j $(NPROCS) --target chess_game_benchmark
...
install-debug install-release: install-%: build-%
	@cd build_$* && \
		cmake --install . -v --component chess_game
...
--in-docker-start-debug --in-docker-start-release: --in-docker-start-%: install-%
	@/home/user/.local/bin/chess_game \
		--config /home/user/.local/etc/chess_game/static_config.yaml
